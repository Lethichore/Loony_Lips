...
var player_words = [] # the words player chooses.
var current_story
var strings #All the text displayed to the player

func _ready():
	set_random_story()
	strings = get_from_json("other_strings.json")
	$Blackboard/StoryText.text = strings["intro text"]
	prompt_player()
...
func get_from_json(filename):
	...
	file.open(filename, File.READ) #Assume that file exists
